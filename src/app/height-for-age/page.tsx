...
import { calculateHeightForAge } from '@/engines/growth';
...
const formSchema = z.object({
  ...
  height: z.coerce.number().min(40, 'Height seems too low').max(220, 'Height seems too high'),
  sex: z.enum(['male', 'female'], { required_error: 'Please select a sex' }),
});

type FormValues = z.infer<typeof formSchema>;

export default function HeightForAgePage() {
  const [result, setResult] = useState<GrowthClassification | null>(null);
  const [chartData, setChartData] = useState<{ age: number; height: number; sex: Sex } | null>(null);
  ...
  const tool = TOOLS.find(t => t.id === 'height-for-age');
  ...
  useEffect(() => {
    ...
    if (activityId) {
      ...
      if (activity && activity.toolId === 'height-for-age' && activity.data) {
  ...
  const onSubmit: SubmitHandler<FormValues> = async (data) => {
    setResult(null);
    try {
      const analysis = await withLoader(calculateHeightForAge(data.age, data.height, data.sex as Sex));
      setResult(analysis);
      setChartData({ age: data.age, height: data.height, sex: data.sex as Sex });
      addActivity({
        id: Date.now().toString(),
        timestamp: Date.now(),
        toolId: 'height-for-age',
        toolName: t('Height-for-Age'),
    ...
    switch(classification) {
        case 'Severely Stunted':
            return {
                ...
                description: "Indicates a significant long-term nutritional deficiency, which can have severe impacts on both physical and cognitive development. This requires urgent and comprehensive medical and nutritional intervention.",
                color: "hsl(var(--destructive))",
            };
        case 'Stunted':
            return {
                title: "Stunting",
                description: "Indicates chronic or recurrent malnutrition, which can lead to impaired growth and development. This condition warrants a thorough nutritional assessment and intervention.",
                color: "hsl(var(--chart-4))",
            };
        case 'Tall':
            return {
                title: "Tall",
                description: "A height-for-age Z-score above +2 is generally considered tall for the age. It's often a reflection of genetic potential but can occasionally indicate certain endocrine disorders. Monitoring growth velocity is important.",
                color: "hsl(var(--chart-2))",
            };
        case 'Very Tall':
            return {
                title: "Very Tall",
                description: "A height-for-age Z-score above +3 is considered very tall. While usually related to genetics, it may warrant an evaluation to rule out underlying medical conditions affecting growth.",
        ...
        default:
            return {
                title: "Normal Height",
                description: "The child's height is within the expected range for their age and sex. Continue to monitor growth as part of routine pediatric care.",
    ...
    <div className="p-4 sm:p-6 lg:p-8">
      ...
      <div className="grid grid-cols-1 gap-8 lg:grid-cols-3">
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Input type="number" step="0.1" placeholder="e.g., 86.4" {...field} />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
        ...
        <div className="lg:col-span-2 space-y-8">
            ...
            <Card>
                <CardHeader>
                    <CardTitle>{t('growth_chart_title')}: {t('Height-for-Age')}</CardTitle>
                </CardHeader>
                <CardContent>
                    {chartData ? <HeightChart {...chartData} /> : <div className="h-[400px] flex items-center justify-center"><p className="text-center text-muted-foreground">{t('enter_data_for_chart')}</p></div>}
                </CardContent>
            </Card>
        </div>
      </div>
